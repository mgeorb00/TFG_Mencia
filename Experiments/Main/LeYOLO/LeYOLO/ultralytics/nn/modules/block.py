...
class BiFPNLayer(nn.Module):
    def __init__(self, channels=96, is_first=False):
        ...
        self.fuse_p4_td  = Conv(channels, channels, 3)
        self.fuse_p3_out = Conv(channels, channels, 3)
        self.fuse_p4_out = Conv(channels, channels, 3)
        self.fuse_p5_out = Conv(channels, channels, 3)
    ...
    def forward(self, inputs):
        ...
        p3_out = self.fuse_p3_out(self._fuse(self.w_p3_out, p4_up, p3))
        p3_dn  = F.max_pool2d(p3_out, 2, 2)
        p4_out = self.fuse_p4_out(self._fuse(self.w_p4_out, p4, p4_td, p3_dn))
        p4_dn  = F.max_pool2d(p4_out, 2, 2)
        p5_out = self.fuse_p5_out(self._fuse(self.w_p5_out, p5, p4_dn))
        # Devolver solo p3_out — Ultralytics no soporta tuplas como output de capas intermedias
        return p3_out
